...
# "Simulation and Optimization of Cocoa Beans Fermentation"           #
#                                                                     #
# The following script contains data from 3 fermentation trials:      #
# ghhp1, brwb1 and brwb2.                                             #
#                                                                     #
...
# Section 4.                                                          #
#                                                                     #
# This code  needs seven arguments for its execution:                 #
#:                                                                    #
...
# reference manual for Stan.                                          #
#                                                                     #
# ad = adapt_delta parameter                                          #
#                                                                     #
# st = step_size parameter                                            #
#                                                                     #
# mt = max_treedepth parameter                                        #
#                                                                     #
# s  = seed parameter                                                 #
#                                                                     #
...
#                                                                     #
# Adaptation of the code writen by Mauricio Moreno-Zambrano           #
# 2021                                                                #
#                                                                     #
#######################################################################

setwd('~/Uni Würzburg/2021-2022/Bachelorarbeit/Simulation CTM')  #directory name, change accordingly if needed
rm(list = ls())
args = c(3, 1, 0.995, .01, 13, 141085, 3000)
...
tmax <- 26/max(x1[,9])
TYmax <- 41.79/max(x1[,9])
Tlabmax <- 46.5/max(x1[,9])
Taabmax <- 43/max(x1[,9])
TYmin <- 3.15/max(x1[,9])
Tlabmin <- 15/max(x1[,9])
Taabmin <- 8/max(x1[,9])
TYopt <- 29.32/max(x1[,9])
Tlabopt <- 37.5/max(x1[,9])
Taabopt <- 35/max(x1[,9])
...
if (args[2]==1){
  ini = function(){
    list(mu1=abs(rnorm(1,0.5,0.2)),mu2=abs(rnorm(1,0.5,0.2)),mu3=abs(rnorm(1,0.5,0.2)),
         ...
         sigma=abs(rnorm(1,0.5,0.2)),b=abs(rnorm(1,0.5,0.2)),
         ...
         Tymax=abs(rnorm(1,TYmax,0.2)),Tlmax=abs(rnorm(1,Tlabmax,0.2)),Tamax=abs(rnorm(1,Taabmax,0.2)),
         Tymin=abs(rnorm(1,TYmin,0.2)),Tlmin=abs(rnorm(1,Tlabmin,0.2)),Tamin=abs(rnorm(1,Taabmin,0.2)),
         Tyopt=abs(rnorm(1,TYopt,0.2)),Tlopt=abs(rnorm(1,Tlabopt,0.2)),Taopt=abs(rnorm(1,Taabopt,0.2)))
  }
}
...
#Stan call:
fit <- stan(paste0("CTM2.stan"),
            data=c("x0","t0","ts","x","T","scl","tmax","TYmax","Tlabmax","Taabmax","TYmin","Tlabmin","Taabmin","TYopt","Tlabopt","Taabopt"),
...
save(fit,file=paste0(lbl,"CTM3.Rsave"))
